...
class ExecutiveEducation2UViewSet(viewsets.ViewSet, EdxOrderPlacementMixin):
    ...
    def _get_receipt_page_url(self, request, user, product):
        """
        Return the link to the receipt page if the product was purchased previously.
        """
    ...
    def _prepare_basket(self, request, product):
        """
        Prepare a basket with the given product for checkout.

        If a current basket exists, all items will be cleared before adding the product.
        """
    ...
    def finish_checkout(self, request):
        ...
        product = get_executive_education_2u_product(partner, sku)
        if not product:
            return Response(f'No Executive Education (2U) product found for SKU {sku}.', status.HTTP_404_NOT_FOUND)
        ...
        try:
            # prepare the basket again to ensure the correct product is in the basket
            basket = self._prepare_basket(request, product)

            if basket.total_excl_tax != 0:
                basket.flush()
                logger.exception(
                    'Failed to create a subsidized order for an Executive Education (2U) product. '
                    'Basket id: [%d], User id: [%d], Product id: [%d]',
                    basket.id, request.user.id, product.id
                )
